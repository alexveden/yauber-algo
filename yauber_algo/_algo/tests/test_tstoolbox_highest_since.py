...
class HighestSinceTestCase(unittest.TestCase):
    def test_highest_since(self):
        ...
        from yauber_algo.algo import highest_since
        ...
        with sc.SanityChecker(algo) as s:
            ...
            #
            s.check_regular(
                array([nan, 2, 3, 4, 5, 6], dtype=np.float),
                func, (
            ...
            s.check_regular(
                array([nan, 5, 5, 3, 3, 1], dtype=np.float),
                func, (
            ...
            s.check_regular(
                array([nan, 5, 5, nan, 5, 1], dtype=np.float),
                func, (
                    array([6, 5, 4, 3, 2, 1], dtype=np.float),  # arr
            ...
            s.check_regular(
                array([nan, 2, 3, nan, 5, 6], dtype=np.float),
                func, (
                    array([1, 2, 3, 4, 5, 6], dtype=np.float),  # arr
                    array([0, 1, 0, nan, 0, 1], dtype=np.float),  # cond
                ),
                suffix='cond_nan_incr'
            )

            s.check_regular(
                array([nan, 2, 3, nan, 3, 6], dtype=np.float),
                func, (
                    array([1, 2, 3, 4, 3, 6], dtype=np.float),  # arr
                    array([0, 1, 0, nan, 0, 1], dtype=np.float),  # cond
                ),
            ...
            s.check_regular(
                array([nan, 2, 3, 1, nan, nan], dtype=np.float),
                func, (
            ...
            s.check_naninf(
                array([nan, nan, 2, 2, nan, nan], dtype=np.float),
                func, (
                    array([1, -inf, 2, 2, 3, 4], dtype=np.float),  # arr
                    array([0, 1, 0, 1, nan, nan], dtype=np.float),  # cond
                ),
                suffix='cond_nan'
            )

            s.check_series(
                pd.Series(array([nan, 2, 3, 4, 5, 6], dtype=np.float)),
                func, (
            ...
            s.check_dtype_bool(
                array([nan, 1, 1, 0, 1, 1], dtype=np.float),
                func, (
